...
# 相对于correlation（局部线性匹配）输出的是相似度的图，这个融合之后输出的是增强后的特征，保留了语义信息和全局信息
...
class DecoderCFALayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,    # 最后特征融合的CFA
                     tgt_mask: Optional[Tensor] = None,
                     memory_mask: Optional[Tensor] = None,
                     tgt_key_padding_mask: Optional[Tensor] = None,
                     memory_key_padding_mask: Optional[Tensor] = None,
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,  # ECA模块负责自相关提升特征表达，CFA模块负责互相关融合两个分支特征 最后再进行一次前向传播，调整一下特征空间。
                     src1_mask: Optional[Tensor] = None,
                     src2_mask: Optional[Tensor] = None,
                     src1_key_padding_mask: Optional[Tensor] = None,
                     src2_key_padding_mask: Optional[Tensor] = None,
...
def build_featurefusion_network(settings): # 构建特征融合网络
    return FeatureFusionNetwork(
        d_model=settings.hidden_dim,  # 256
        dropout=settings.dropout,  # 0.1
        nhead=settings.nheads,  # 8
        dim_feedforward=settings.dim_feedforward,  # 2048
        num_featurefusion_layers=settings.featurefusion_layers  # 4
    )
