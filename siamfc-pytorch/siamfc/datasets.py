...
class Pair(Dataset):
    """继承了Dataset类，所以要overwrite __getitem__和__len__方法：
        __getitem__：这个方法就是通过index索引返回item = (z, x, box_z, box_x)，然后经过transforms返回一对pair(z, x)
    ...
    def _sample_pair(self, indices):
        """如果有效索引大于2个的话，就从中随机挑选两个索引，这里取的间隔不超过T=100。  100帧"""
        n = len(indices)
        assert n > 0
