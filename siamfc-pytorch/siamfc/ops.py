...
def crop_and_resize(img, center, size, out_size,
    ...
    # 传入size和center计算出角落坐标形式的正方形patch，（ymin，xmin，ymax，xmax）
    size = round(size)
    ...
    # pad image if necessary  因为扩大的坐标有可能会超出原来的图片
    pads = np.concatenate((
        -corners[:2], corners[2:] - img.shape[:2]))
    npad = max(0, int(pads.max()))  # 找出超出最大的那个长度npad，然后在原图周围填充
    if npad > 0:
        img = cv2.copyMakeBorder(
            img, npad, npad, npad, npad,
            border_type, value=border_value)

    # 因为原图像增大了，所以corner相对坐标也变了
...
if __name__ == '__main__':
    ...
    center = np.array([180, 580])
    size = 125
    out_size = 255
    patch = crop_and_resize(img, center, size, out_size)
